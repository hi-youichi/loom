Feature: loom CLI ACP client

  The `loom --acp` mode is a CLI-facing ACP client.

  Scenario: create a session through CLI ACP mode
    Given a loom server is running for ACP BDD tests
    When I run loom with --acp and --json
  ...
  Scenario: resume a session through CLI ACP mode
    ...
    When I run loom with --acp and that session id
    Then the command exits successfully
  ...
  Scenario: execute a prompt through CLI ACP mode
    ...
    When I run loom with --acp and a prompt
    Then the command exits successfully
