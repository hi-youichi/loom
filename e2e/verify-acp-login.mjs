...
ws.onopen = () => ws.send('{"jsonrpc":"2.0","id":1,"method":"_anureo.dev/auth/login","params":{"password":"e2e-test-pw","trustDevice":false}}');
ws.onmessage = (ev) => {
  ...
  ws.send(JSON.stringify({ jsonrpc: '2.0', id: 2, method: '_anureo.dev/auth/authenticate', params: { sessionToken: token } }));
  ws.onmessage = (ev2) => {
