...
test("BDD: one anureo server supports alternating and concurrent ACP sessions", { timeout: 30_000 }, async () => {
  const fixture = await startFixture();
  const workspaceA = await realpath(await mkdtemp(path.join(tmpdir(), "anureo-acp-a-")));
  const workspaceB = await realpath(await mkdtemp(path.join(tmpdir(), "anureo-acp-b-")));
...
test("BDD: a second ACP connection does not replace the first", { timeout: 20_000 }, async () => {
  const fixture = await startFixture();
  const workspace = await realpath(await mkdtemp(path.join(tmpdir(), "anureo-acp-multi-")));
...
test("BDD: session metadata survives a anureo server restart", { timeout: 25_000 }, async () => {
  const otherDir = await mkdtemp(path.join(tmpdir(), "anureo-acp-other-"));
  const workspace = await realpath(await mkdtemp(path.join(tmpdir(), "anureo-acp-persist-")));
