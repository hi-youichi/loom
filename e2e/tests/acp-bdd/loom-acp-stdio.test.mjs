...
const loomBinary = process.env.LOOM_BIN ?? defaultLoomBinary();
const timeoutMs = Number(process.env.ACP_BDD_TIMEOUT_MS ?? 15_000);

function defaultLoomBinary() {
  return process.platform === "win32"
    ? path.join(cargoTargetDir, "debug", "loom.exe")
    : path.join(cargoTargetDir, "debug", "loom");
}

function requireLoomBinary() {
  if (!loomBinary) {
    throw new Error("Set LOOM_BIN to a built loom executable");
  }
}
...
async function startServer(env) {
  ...
  const line = await waitForLine(
    child.stdout,
    (value) => value.includes("loom server listening on http://"),
    "loom server address",
  );
...
function initializeParams() {
  return {
    protocolVersion: 1,
    clientInfo: { name: "loom-node-bdd", version: "0.1.0" },
    clientCapabilities: {},
  };
}
