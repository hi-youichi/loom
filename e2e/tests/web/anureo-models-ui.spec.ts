/**
 * e2e/tests/web/anureo-models-ui.spec.ts
 *
 * anureo Backend Models UI Verification
 * 验证 anureo 前端的模型选择器能正确显示 anureo backend 返回的模型列表
 *
 * 前置条件：
 * 1. anureo-server 运行在 http://127.0.0.1:18081
 *    cargo run -p anureo-server -- serve --host 127.0.0.1 --port 18081
 *
 * 2. anureo 前端运行在 http://localhost:5173
 *    cd anureo-feat-dev
 *    $env:ANUREO_HOST = "http://127.0.0.1:18081"
 *    $env:ANUREO_PORT = "3200"
 *    bun run dev:web:full
 *
 * 运行：
 *    cd e2e
 *    npx playwright test anureo-models-ui.spec.ts
 */

import { test, expect } from "@playwright/test";

const ANUREO_SERVER_URL = process.env.ANUREO_SERVER_URL ?? "http://127.0.0.1:18081";
const ANUREO_URL = process.env.ANUREO_URL ?? "http://localhost:5173";

// ─── API 测试（不需要 UI）─────────────────────────────────────────────────────

test.describe("anureo Backend API", () => {
  test("/api/model 返回正确格式和模型列表", async ({ request }) => {
    const resp = await request.get(`${ANUREO_SERVER_URL}/api/model`);
    ...
    console.log("anureo API models count:", models.length);
...
test.describe("anureo Backend Models UI", () => {
  test.beforeEach(async ({ page }) => {
    // 导航到 anureo 并等待加载
    await page.goto(ANUREO_URL, { waitUntil: "domcontentloaded" });
  ...
  test("模型选择器能显示 anureo backend 返回的模型", async ({ page }) => {
    // 1. 新建会话
    ...
    // 6. 截图记录
    await page.screenshot({ path: "anureo-models-dropdown.png" });
  });
  ...
  test("选择模型后会话使用该模型", async ({ page }) => {
    ...
    // 5. 截图记录
    await page.screenshot({ path: "anureo-model-selected.png" });
  });
});
