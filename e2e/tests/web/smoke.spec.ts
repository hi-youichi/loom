 ...
 * - mock-opencode: 拦截 /api/sessions 和 /api/models 请求（返回空列表）
 *
...
// 合并所有 fixtures
// 使用 mock-opencode fixture 的 page 拦截（自动注册 route handlers + token 注入）
// + auth fixture 的 loginWithPassword。
// 不要在这里定义自己的 page fixture — 会 shadow mock-opencode 的拦截器。
