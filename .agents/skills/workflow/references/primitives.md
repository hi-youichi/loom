...
## agent(opts) -> result
    ...
    opts:   { prompt=<string, required>,     -- instructions for the subagent (see references/agent-prompts.md)
              ...
              working_folder=<string?> }     -- per-agent directory override
    ...
    `working_folder` is resolved relative to the workflow's working directory
    by default. An absolute path is also accepted. The workflow working
    directory itself is set by the MCP `workflow_execute.working_dir` option.
