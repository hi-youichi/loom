...
/// Application name used under XDG state_home (e.g. state_home/graphweave/todos.json).
const XDG_APP_NAME: &str = "graphweave";
...
///
/// Resolves to e.g. `$XDG_STATE_HOME/graphweave/todos.json` (Linux: `~/.local/state/graphweave/todos.json`).
...
mod tests {
    ...
    fn todo_file_path_uses_xdg_state_home() {
        ...
        assert!(path.ends_with(std::path::Path::new("graphweave").join("todos.json")));
        assert_eq!(path.parent().unwrap().file_name().unwrap(), "graphweave");
    ...
    fn constants_match_docs() {
        assert_eq!(XDG_APP_NAME, "graphweave");
