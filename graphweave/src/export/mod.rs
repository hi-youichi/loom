...
/// Aligned with [EXPORT_SPEC](https://github.com/graphweave/graphweave/blob/main/docs/EXPORT_SPEC.md) §2.
    ...
    S: Serialize + Clone + Send + Sync + Debug + 'static,
{
    let obj = match ev {
        ...
        StreamEvent::Messages {
            ...
            metadata:
                StreamMetadata { graphweave_node },
        } => json!({
            "Messages": {
                "chunk": { "content": content },
                "metadata": { "graphweave_node": graphweave_node }
            }
        }),
    ...
    fn messages_format() {
        let ev: StreamEvent<DummyState> = StreamEvent::Messages {
            ...
            metadata: StreamMetadata {
                graphweave_node: "think".to_string(),
            },
        };
        let v = stream_event_to_format_a(&ev).unwrap();
        assert_eq!(v["Messages"]["chunk"]["content"], "hello");
        assert_eq!(v["Messages"]["metadata"]["graphweave_node"], "think");
    }
}
