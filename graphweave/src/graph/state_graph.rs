    ...
    S: Clone + Send + Sync + Debug + 'static,
{
    ...
    /// use graphweave::channels::FieldBasedUpdater;
    /// use std::sync::Arc;
    ///
    /// #[derive(Clone, Debug)]
    ...
    ///
    /// ```rust,no_run
    /// use graphweave::graph::{StateGraph, RetryPolicy};
    ...
    ///
    /// ```rust,no_run
    /// use graphweave::graph::{StateGraph, DefaultInterruptHandler};
    /// use std::sync::Arc;
    ///
    /// let graph = StateGraph::<String>::new()
    ...
    ///
    /// ```rust,ignore
    /// use graphweave::graph::{StateGraph, END};
