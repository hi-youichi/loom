//! Streaming types for GraphWeave runs.
//!
...
//!
//! ```rust,ignore
//! use graphweave::stream::{StreamWriter, StreamMode};
//!
...
///
/// ```rust,ignore
/// use graphweave::stream::ToolStreamWriter;
/// use serde_json::json;
///
...
pub struct StreamMetadata {
    /// GraphWeave node id that produced the message.
    pub graphweave_node: String,
}
...
///
/// ```rust,ignore
/// use graphweave::stream::StreamWriter;
///
/// async fn run_with_context(&self, state: S, ctx: &RunContext<S>) -> Result<(S, Next), AgentError> {
///     let writer = StreamWriter::from_context(ctx);
///     
