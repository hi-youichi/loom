...
use graphweave::tools::{AggregateToolSource, TOOL_SKILL};
...
async fn skill_load_by_name_with_md_extension() {
    ...
    let skills_dir = dir.path().join(".graphweave").join("skills");
...
async fn skill_missing_name_returns_error() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(dir.path().join(".graphweave").join("skills")).unwrap();
    let agg = aggregate_with_file_tools(&dir);
