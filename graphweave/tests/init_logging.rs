...
//! from the graphweave library (e.g. `tracing::debug!` in `openai.rs`) are printed
...
//!
//! ```bash
//! RUST_LOG=graphweave=debug cargo test -p graphweave -- --nocapture
//! RUST_LOG=debug cargo test -p graphweave openai_sse -- --nocapture
//! ```
