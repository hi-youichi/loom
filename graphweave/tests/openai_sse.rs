...
fn parse_request_errors_when_no_user_message() {
    ...
    assert!(matches!(err, graphweave::ParseError::NoUserMessage));
}
...
fn parse_request_with_working_folder_and_approval_policy_sets_helve_config() {
    ...
    assert_eq!(
        ...
        Some(graphweave::ApprovalPolicy::DestructiveOnly)
    );
...
fn parse_request_invalid_approval_policy_errors() {
    ...
    assert!(matches!(
        err,
        graphweave::ParseError::InvalidApprovalPolicy(_)
    ));
}
