...
async fn test_model_persistence_across_restarts() {
    ...
    assert_eq!(saved_model, selected_model, "Model should be saved correctly");
    ...
    assert_eq!(restored_model, selected_model, "Model should persist across restarts");
    
    // 7. 验证恢复的模型对应的tier
    let tier = mock_server.get_tier_from_model(&restored_model);
    assert_eq!(tier, Some("high".to_string()), "Restored model should map to correct tier");
    
    println!("✅ Model persistence test passed - model selection correctly persists across restarts");
}
...
async fn test_model_persistence_file_format() {
    ...
    for model in test_models {
        ...
        assert_eq!(read_model, model, "Model {} should be read back correctly", model);
        
        // 验证文件内容格式（应该只是模型名称，没有额外内容）
        let file_content = std::fs::read_to_string(test_env.last_model_path())
            .expect("Should be able to read file");
        assert_eq!(file_content.trim(), model, "File content should be just the model name");
    }
...
async fn test_model_persistence_overwrites() {
    ...
    assert_eq!(second_read, second_model, "Second model should overwrite first");
    
    // 3. 验证新模型对应的tier
    let new_tier = mock_server.get_tier_from_model(&second_read);
    assert_eq!(new_tier, Some("high".to_string()), "New model should map to high tier");
...
async fn test_model_persistence_with_different_tiers() {
    ...
    for (model, expected_tier) in test_cases {
        ...
        assert_eq!(tier, Some(expected_tier.to_string()), 
                   "Model {} should map to {} tier", model, expected_tier);
    }
    
    println!("✅ Multi-tier persistence test passed - models from different tiers persist correctly");
}