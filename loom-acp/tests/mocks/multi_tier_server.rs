...
impl MultiTierMockServer {
    pub async fn new() -> Self {
        ...
        response_templates.insert("high".to_string(), Self::create_tiered_response("high", "High tier response"));
        response_templates.insert("medium".to_string(), Self::create_tiered_response("medium", "Medium tier response"));
        response_templates.insert("low".to_string(), Self::create_tiered_response("low", "Low tier response"));
    ...
    async fn setup_endpoints(&self) {
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(self.create_chat_completion_response()))
            .mount(&self.server)
            .await;
...
mod tests {
    ...
    async fn test_model_mapping() {
        let server = MultiTierMockServer::new().await;
        assert_eq!(server.get_tier_from_model("gpt-4-high"), Some("high".to_string()));
        assert_eq!(server.get_tier_from_model("gpt-3.5-medium"), Some("medium".to_string()));
        assert_eq!(server.get_tier_from_model("gpt-3.5-low"), Some("low".to_string()));
    }
}