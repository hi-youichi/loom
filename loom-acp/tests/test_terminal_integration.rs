...
mod tests {
    ...
    async fn test_terminal_lifecycle() {
        ...
        let term_id = manager.create_terminal(
            "echo".to_string(),
        ...
        manager.update_status(&term_id, TerminalStatus::Completed { exit_code: 0 }).await;
    ...
    async fn test_terminal_output_streaming() {
        ...
        for i in 0..5 {
            manager.append_output(&term_id, &format!("Line {}\n", i)).await;
        }
    ...
    async fn test_multiple_terminals() {
        ...
        let term1 = manager.create_terminal("cmd1".to_string(), None, None).await.unwrap();
        let term2 = manager.create_terminal("cmd2".to_string(), None, None).await.unwrap();
    ...
    async fn test_terminal_status_transitions() {
        ...
        let term_id = manager.create_terminal("test".to_string(), None, None).await.unwrap();
        
        assert_eq!(manager.get_status(&term_id).await, Some(TerminalStatus::Running));
        
        manager.update_status(&term_id, TerminalStatus::Completed { exit_code: 0 }).await;
        assert_eq!(manager.get_status(&term_id).await, Some(TerminalStatus::Completed { exit_code: 0 }));
        
        let term_id2 = manager.create_terminal("fail".to_string(), None, None).await.unwrap();
        manager.update_status(&term_id2, TerminalStatus::Failed { error: "Error".to_string() }).await;
        assert_eq!(manager.get_status(&term_id2).await, Some(TerminalStatus::Failed { error: "Error".to_string() }));
    }
