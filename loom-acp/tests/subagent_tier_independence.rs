...
use common::config_helpers::{create_subagent_config, create_agent_config};
...
async fn test_subagent_tier_independence() {
    ...
    assert_eq!(acp_tier, Some("high".to_string()), "ACP should use high tier");
    
    // 6. 验证子代理配置保持为 low tier
    let subagent_config_path = test_env.loom_home.join(".loom/agents/product-manager/config.yaml");
    assert!(subagent_config_path.exists(), "Subagent config should exist");
    
    let subagent_config = std::fs::read_to_string(&subagent_config_path)
        .expect("Should be able to read subagent config");
    assert!(subagent_config.contains("low"), "Subagent should still have low tier config");
    
    // 7. 验证子代理对应的模型映射
    let subagent_model = "gpt-3.5-low";
    let subagent_tier = mock_server.get_tier_from_model(subagent_model);
    assert_eq!(subagent_tier, Some("low".to_string()), "Subagent should map to low tier");
    
    // 8. 验证ACP和子代理使用不同的tier
    assert_ne!(acp_tier, subagent_tier, "ACP and subagent should use different tiers");
...
async fn test_multiple_subagents_different_tiers() {
    ...
    for (agent_name, expected_tier, expected_model) in tiers {
        ...
        assert_eq!(tier, Some(expected_tier.to_string()), "{} model should map to {} tier", agent_name, expected_tier);
    }
...
async fn test_acp_selection_doesnt_affect_subagent_configs() {
    ...
    // 读取原始配置
    let original_config = std::fs::read_to_string(test_env.loom_home.join(".loom/agents/product-manager/config.yaml"))
    ...
    // 验证子代理配置文件未被修改
    let current_config = std::fs::read_to_string(test_env.loom_home.join(".loom/agents/product-manager/config.yaml"))
        .expect("Should be able to read current config");
    
    assert_eq!(original_config, current_config, "Subagent config should not be modified by ACP selection");
    
    println!("✅ Config isolation test passed - ACP selection doesn't modify subagent config files");
}