...
//!
//! The actual `ModelRegistry` runtime (caching, fetching from models.dev)
//! and `create_llm_client` / `create_llm_provider` factory functions
//! remain in the `loom` crate because they depend on `ChatOpenAI`,
//! `ChatOpenAICompat`, and `model_spec`.
