...
fn build_configured_interrupt(task: &PreparedTask, when: &str) -> Interrupt {
    Interrupt::with_id(
        serde_json::json!({
...
fn reserved_interrupt_from_task(
...
) -> Option<(Interrupt, InterruptRecord)> {
    let value = task
        .writes
        .iter()
    ...
    let interrupt = Interrupt::with_id(value.clone(), interrupt_id);
    let namespace = interrupt_namespace(&interrupt, default_namespace);
    let record = interrupt_record_from_task(&task.prepared, &interrupt, namespace);
