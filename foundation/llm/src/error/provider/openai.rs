...
mod tests {
    ...
    fn maps_401_to_auth_failed() {
        let err = parse(401, br#"{"error":{"message":"bad key","type":"invalid_request_error"}}"#);
        assert_eq!(err.kind, ErrorKind::AuthFailed);
        assert!(matches!(err.retry_policy, RetryPolicy::NoRetry { action: _ }));
    }

    #[test]
    fn maps_429_rate_limit_to_retryable() {
        let err = parse(429, br#"{"error":{"message":"rate limit reached","type":"rate_limit_error"}}"#);
    ...
    fn maps_429_insufficient_quota_to_quota_exhausted() {
        let err = parse(429, br#"{"error":{"message":"insufficient_quota","type":"insufficient_quota"}}"#);
    ...
    fn maps_content_filter_type() {
        let err = parse(400, br#"{"error":{"message":"content filtered","type":"content_filter"}}"#);
    ...
    fn keeps_raw_message_and_request_id() {
        let err = parse(500, br#"{"error":{"message":"boom","type":"api_error"},"request_id":"req-1"}"#);
