            ...
            for (field, op) in filter {
                hits.retain(|h| {
                    ...
                    match (field_value, op) {
                        (Some(v), crate::FilterOp::Eq(expected)) => v == expected,
                        (Some(v), crate::FilterOp::Ne(expected)) => v != expected,
                        (Some(v), crate::FilterOp::Gt(expected)) => {
                            compare_json(v, expected) == Some(std::cmp::Ordering::Greater)
                        }
                        (Some(v), crate::FilterOp::Gte(expected)) => {
                            matches!(
                                compare_json(v, expected),
                                Some(std::cmp::Ordering::Greater | std::cmp::Ordering::Equal)
                            )
                        }
                        (Some(v), crate::FilterOp::Lt(expected)) => {
                            compare_json(v, expected) == Some(std::cmp::Ordering::Less)
                        }
                        (Some(v), crate::FilterOp::Lte(expected)) => {
                            matches!(
                                compare_json(v, expected),
