//! Wraps graphweave::run_agent with stderr display callback.
...
use graphweave::{RunCmd, RunOptions, StreamEvent};
    ...
    if opts.output_json {
        ...
        let on_event = Box::new(move |ev: AnyStreamEvent| {
            match ev.to_format_a() {
                ...
                Err(e) => {
                    eprintln!("graphweave: failed to serialize stream event to JSON: {}", e);
                    if let Ok(mut vec) = events_clone.lock() {
...
async fn print_loaded_tools(config: &graphweave::ReactBuildConfig) -> Result<(), RunError> {
    let ctx = build_react_run_context(config)
        .await
        .map_err(|e| RunError::Build(graphweave::BuildRunnerError::Context(e)))?;
    let tools = ctx.tool_source.list_tools().await.map_err(|e| {
        RunError::Build(graphweave::BuildRunnerError::Context(
            graphweave::AgentError::ExecutionFailed(e.to_string()),
        ))
    })?;
