    ...
    W::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    match req {
        ...
        ClientRequest::Ping(r) => {
            send_response(write, &ServerResponse::Pong(graphweave::PongResponse { id: r.id }))
                .await?;
        }
    }
    Ok(())
...
async fn handle_tools_list(r: graphweave::ToolsListRequest) -> ServerResponse {
    let id = r.id.clone();
    let opts = RunOptions {
        message: String::new(),
        working_folder: None,
...
async fn handle_tool_show(r: graphweave::ToolShowRequest) -> ServerResponse {
    let id = r.id.clone();
    let opts = RunOptions {
        message: String::new(),
        working_folder: None,
