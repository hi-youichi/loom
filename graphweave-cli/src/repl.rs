...
use graphweave_cli::{RunBackend, RunCmd, RunError, RunOptions};
...
pub async fn run_repl_loop(
    ...
    json_pretty: bool,
    stream_out: graphweave_cli::StreamOut,
    ...
    loop {
        ...
        match run_one_turn(backend, &opts, cmd, stream_out.clone()).await {
            Ok(graphweave_cli::RunOutput::Json { events, reply }) => {
            ...
            Ok(graphweave_cli::RunOutput::Reply(reply)) => {
                if json_stream {
...
pub async fn run_one_turn(
    ...
    cmd: &Command,
    stream_out: graphweave_cli::StreamOut,
) -> Result<graphweave_cli::RunOutput, RunError> {
