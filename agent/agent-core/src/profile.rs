...
/// Built-in agent-builder: meta-agent that creates new agent profiles.
...
/// Built-in explore agent: file search specialist for codebase navigation.
...
pub fn parse_front_matter(content: &str) -> (&str, Option<String>) {
    const DELIM: &str = "---";
...
pub fn resolve_extends_path(parent: &Path, extends: &str) -> Option<PathBuf> {
...
pub fn merge_role(base: Option<RoleConfig>, over: Option<RoleConfig>) -> Option<RoleConfig> {
    match (base, over) {
        (None, over) => over,
        (base, None) => base,
...
pub fn merge_profiles(mut base: AgentProfile, over: AgentProfile) -> AgentProfile {
...
pub fn merge_tools_config(base: ToolsConfig, over: ToolsConfig) -> ToolsConfig {
...
pub fn merge_disabled_lists(a: Option<Vec<String>>, b: Option<Vec<String>>) -> Option<Vec<String>> {
...
/// Load a profile from a name, returning the profile and source classification.
...
pub fn load_builtin_profile(name: &str) -> Option<AgentProfile> {
...
"#;
    ...
    #[test]
    fn builtin_dev_agent_has_content() {
    ...
    #[test]
    fn builtin_agent_builder_has_content() {
    ...
    #[test]
    fn load_profile_by_name_returns_builtin_dev() {
        let (profile, source) = load_profile_by_name("dev").unwrap();
        assert_eq!(profile.name, "dev");
        assert_eq!(source, ProfileSource::BuiltIn);
    }

    #[test]
    fn load_profile_by_name_returns_none_for_unknown_in_empty_dir() {
        let dir = tempfile::tempdir().unwrap();
