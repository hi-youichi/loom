...
impl ReActState {
    pub fn apply_think(
        ...
        let fixed_tool_calls: Vec<ToolCall> = tool_calls
            .iter()
            .map(|tc| {
                if tc.id.as_deref().is_none_or(|s| s.is_empty()) {
                    ToolCall {
                        id: Some(format!("call_{}", uuid6())),
                        ..tc.clone()
                    }
                } else {
                    tc.clone()
            ...
            .collect();
        let atc: Vec<AssistantToolCall> = fixed_tool_calls
            .iter()
            .map(|tc| AssistantToolCall {
                id: tc.id.clone().unwrap(),
                name: tc.name.clone(),
...
impl From<NormalizedToolOutput> for ToolResult {
    fn from(n: NormalizedToolOutput) -> Self {
        Self {
            ...
            observation_text: if n.observation_text.is_empty() {
                None
            } else {
                Some(n.observation_text)
            },
            display_text: if n.display_text.is_empty() {
                None
            } else {
                Some(n.display_text)
            },
