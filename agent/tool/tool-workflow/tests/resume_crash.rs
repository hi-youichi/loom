...
async fn resume_and_assert(tmp: &tempfile::TempDir, prior_dir: &str, expected_count: usize) {
...
async fn t1_crash_between_phases() {
    ...
    resume_and_assert(&tmp, &prior_dir, 2).await;
}
...
async fn t3_crash_between_agents_in_phase() {
    ...
    resume_and_assert(&tmp, &prior_dir, 3).await;
}
...
async fn t8_crash_near_completion() {
    ...
    resume_and_assert(&tmp, &prior_dir, 1).await;
}
