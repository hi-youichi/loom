...
## Example 1: per-module refactoring (static decomposition)

Hardcoded list of modules. Each module goes through `analyze → refactor → verify`.
...
## Example 2: whole-crate refactoring (dynamic enumeration, nested loops)

The number of subsystems and modules isn't known up front — agent enumerates
them first.
...
## Example 3: adversarial verification (cross-check findings via voting)

Three voters per item; keep findings whose approval rate ≥ 0.7; iterate up to
3 rounds or until convergence.
