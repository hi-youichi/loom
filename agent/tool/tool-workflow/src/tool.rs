...
impl Tool for WorkflowTool {
    ...
    async fn call(
        ...
        match action {
            ...
            // T-03: new wiring (instance artefacts + compact summary).
            // Kept as a distinct dispatch arm so legacy `run` keeps
            // its exact behaviour for any caller that depends on the
            // current text shape. The "execute" arm routes to a
            // separate handler that owns the post-T-02 path layout.
            // T-02 will rename `"run"` → `"execute"` in this match
            // (and add the deprecation alias); the merge step resolves
            // the duplicate arm.
