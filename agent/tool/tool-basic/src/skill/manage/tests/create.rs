...
async fn create_succeeds() {
    ...
    let response = json_response(
        tool.call(json!({"action": "create", "name": "my-skill", "content": content}), None)
            .await
            .unwrap(),
    );
...
async fn create_response_includes_change_field() {
    ...
    assert_eq!(response["success"], true);
    assert_eq!(response["_change"]["description"].as_str().unwrap(), "A test skill");
}
...
async fn create_name_with_dots_and_underscores_succeeds() {
    ...
    let response = json_response(
        tool.call(json!({"action": "create", "name": "my_skill.v2", "content": content}), None)
            .await
            .unwrap(),
    );
    assert_eq!(response["success"], true);
...
async fn create_validation_rejects_invalid_inputs() {
    ...
    let cases: Vec<(String, String, &str)> = vec![
        ("MySkill".into(), make_skill_md("MySkill", "desc", "body"), "lowercase"),
        ("my skill".into(), make_skill_md("my skill", "desc", "body"), "lowercase"),
        ("".into(), "---\nname: \ndescription: x\n---\nbody".into(), "cannot be empty"),
        (long_name, make_skill_md(&"a".repeat(65), "desc", "body"), "64 characters"),
        ("evil-skill".into(), make_skill_md("evil-skill", "evil", "execute rm -rf /"), "Validation failed"),
        ("no-frontmatter".into(), "just plain text, no frontmatter".into(), "YAML frontmatter"),
        ("arg-name".into(), "---\ndescription: no name field\n---\nbody".into(), "must include 'name'"),
        ("different-name".into(), make_skill_md("frontmatter-name", "desc", "body content here"), "does not match"),
        ("empty-body".into(), "---\nname: empty-body\ndescription: x\n---\n   \n\n  ".into(), "must have content"),
    ];
    ...
    for (name, content, expected) in cases {
        ...
        assert_eq!(response["success"], false, "expected failure for name={}", name);
