...
async fn spec_matches_reference_schema() {
    ...
    let actions: Vec<&str> = spec
        ...
        .collect();
    assert_eq!(actions, vec!["create", "patch", "edit", "delete", "write_file", "remove_file"]);
    ...
    for key in &["action", "name", "content", "old_string", "new_string", "replace_all", "category", "file_path", "file_content", "absorbed_into"] {
        assert!(props.contains_key(*key), "missing property: {}", key);
    }
    for removed in &["body", "description", "triggers"] {
        assert!(!props.contains_key(*removed), "should not contain: {}", removed);
    }
}
...
async fn unknown_action_returns_error() {
    ...
    let err = tool.call(json!({"action": "invalid", "name": "x"}), None).await.unwrap_err();
...
async fn write_origin_guard_set_during_call() {
    ...
    tool.call(json!({"action": "create", "name": "guard-test", "content": content}), None)
        .await
        .unwrap();
