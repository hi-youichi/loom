...
fn validate_frontmatter_missing_name() {
    ...
    assert!(err.warnings.iter().any(|w| w.message.contains("must include 'name'")));
}

#[test]
fn validate_frontmatter_missing_description() {
    let err = validate_frontmatter("---\nname: foo\n---\nbody").unwrap_err();
    assert!(err.warnings.iter().any(|w| w.message.contains("must include 'description'")));
}

#[test]
fn validate_frontmatter_no_frontmatter() {
    let err = validate_frontmatter("just text").unwrap_err();
    assert!(err.warnings.iter().any(|w| w.message.contains("YAML frontmatter")));
}

#[test]
fn validate_frontmatter_unclosed_frontmatter() {
    let err = validate_frontmatter("---\nname: foo\ndescription: bar\nbody without closing").unwrap_err();
    assert!(err.warnings.iter().any(|w| w.message.contains("not closed")));
}
