...
//!
//! Run with: `cargo run -p tool-luft --example validate_skill`
...
fn main() {
    println!("=== Luft builtin skill: runtime validation ===\n");

    // 1. LuftTool exposes the builtin skill
    let tool = LuftTool::new(AgentConfig::default());
    let skill = tool
        .builtin_skill()
        .expect("LuftTool must expose builtin_skill()");
    println!("[1] LuftTool::builtin_skill()");
    ...
    let (entry_source, entry_name, embedded_size, requires_tools) = {
    ...
    let has_example = content.contains("## Example: analyze");
    println!("\n[5] load_skill_with_dir(\"luft-workflow-dsl\")");
    ...
    println!("    has example section:  {}", has_example);

    // 6. available_skills_prompt includes it (drives nudge system)
    let prompt = registry.available_skills_prompt();
    let in_prompt = prompt.contains("luft-workflow-dsl");
    println!("\n[6] available_skills_prompt");
    println!("    contains 'luft-workflow-dsl': {}", in_prompt);
    ...
    // Insert at front so it has priority over the existing builtin entry.
    ...
    let tool2 = LuftTool::new(AgentConfig::default());
    ...
    // Find the user override entry (Source::Project) — should be first
    ...
    // 8. apply_toolset_filters: hides when luft tool missing
    let mut registry2 = SkillRegistry::empty();
    if let Some(s) = LuftTool::new(AgentConfig::default()).builtin_skill() {
    ...
    println!(
        "    builtin skill hidden: {} (expect true since 'luft' missing)",
        filtered_count == 0
    );
