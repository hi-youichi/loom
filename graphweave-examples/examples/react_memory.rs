...
impl LlmClient for MemoryMockLlm {
    async fn invoke(
        &self,
        messages: &[Message],
    ) -> Result<LlmResponse, graphweave::error::AgentError> {
...
impl ToolSource for MemoryToolSource {
    async fn list_tools(&self) -> Result<Vec<ToolSpec>, graphweave::tool_source::ToolSourceError> {
        Ok(vec![
            ToolSpec {
    ...
    ) -> Result<ToolCallContent, graphweave::tool_source::ToolSourceError> {
        match name {
            "save_memory" => {
                ...
                self.store
                    ...
                    .await
                    .map_err(|e| {
                        graphweave::tool_source::ToolSourceError::Transport(e.to_string())
                    })?;
                Ok(ToolCallContent {
            ...
            _ => Err(graphweave::tool_source::ToolSourceError::NotFound(format!(
                "Unknown tool: {}",
                name
            ))),
        }
...
impl Node<MemoryReActState> for MemoryThinkNode {
    ...
    ) -> Result<(MemoryReActState, Next), graphweave::error::AgentError> {
    ...
    async fn run(
        ...
        for tc in &state.tool_calls {
            ...
            let content = self
                ...
                .await
                .map_err(|e| graphweave::error::AgentError::ExecutionFailed(e.to_string()))?;
