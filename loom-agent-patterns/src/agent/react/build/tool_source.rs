...
pub(crate) async fn build_tool_source(
    ...
    aggregate
        ...
        .await;
    // aggregate.register_sync(Box::new(ListAgentsTool::new())); // TODO: list_agents moved to loom
    ...
    if let Some(ref wf) = config.working_folder {
        // TODO: Fix skill_registry type mismatch between loom::skill::SkillRegistry and loom_skill::discovery::SkillRegistry
        register_file_tools(aggregate.as_ref(), wf, None, Some(SkillUsageStore::new(&wf.join(".loom/skills"))))
            .map_err(to_agent_error)?;

        let db_path = loom_home().join("tasks").join("tasks.db");
        let db_dir = db_path.parent().unwrap();
        let _ = std::fs::create_dir_all(db_dir);
        if let Ok(db) = task_core::TaskDb::open(&db_path).await {
            task::register_task_tools(&aggregate, Arc::new(db)).await;
        }
    }
    aggregate.register_sync(Box::new(BatchTool::new(Arc::clone(&aggregate))));
    aggregate.register_sync(Box::new(LspTool::default()));
