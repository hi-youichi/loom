                    ...
                    "agents": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "model_tier": {
                                    "type": "string",
                                    "enum": ModelTier::variants().to_vec(),
...
tracing::debug!(agent = %agent_name, "Starting sub-agent execution");
        ...
        let any_sender: Option<std::sync::Arc<dyn Fn(AnyStreamEvent) + Send + Sync>> =
            loom_sender.clone().map(|ls| {
        ...
        let on_event = Some(move |event: loom_stream::StreamEvent<loom_types::state::ReActState>| {
            ...
            //         sender(loom_types::cli_run::AnyStreamEvent::React(json_value));
            //     }
            // }
        });
...
///
/// When `isolation: "worktree"` is specified in args (or the profile defaults to worktree
/// isolation), a git worktree is created before execution and cleaned up after.
async fn invoke_single_agent(
    ...
    // Determine if worktree isolation should be used:
    // Priority: tool arg > profile config > none
    let use_worktree = isolation_arg.as_deref() == Some("worktree")
        || (isolation_arg.is_none() && profile.isolation.as_deref() == Some("worktree"));

    // If worktree isolation is active, create a worktree and override working folder
    ...
    // Override working folder to worktree path if created
    ...
    let mut sub_config =
        build_config_from_profile(&profile, base_config, effective_working_folder.as_deref());
