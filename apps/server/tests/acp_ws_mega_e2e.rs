...
async fn request(
    ...
    socket
        .send(Message::Text(json!({"jsonrpc":"2.0","id":id,"method":method,"params":params}).to_string()))
        .await
        .expect("send request");
    loop {
...
async fn acp_websocket_mega_initialize_session_and_reconnect() {
    ...
    let initialize = request(&mut first, 1, "initialize", json!({
        "protocolVersion": 1,
        "clientInfo": {"name":"ws-mega", "version":"0.1"},
        "capabilities": {"session": {"list": {}}}
    })).await;
    assert!(initialize.get("result").is_some(), "initialize response: {initialize}");

    let created = request(&mut first, 2, "session/new", json!({
        "cwd": std::env::current_dir().expect("cwd").to_string_lossy(),
        "mcpServers": []
    })).await;
    let session_id = created["result"]["sessionId"].as_str().expect("session id").to_owned();
    first.close(None).await.expect("close first connection");

    let (mut second, _) = connect_async(&url).await.expect("reconnect ACP WS");
    let initialize_again = request(&mut second, 3, "initialize", json!({
        "protocolVersion": 1,
        "clientInfo": {"name":"ws-mega", "version":"0.1"},
        "capabilities": {"session": {"list": {}}}
    })).await;
    ...
    let loaded = request(&mut second, 4, "session/load", json!({
        "sessionId": session_id,
        "cwd": std::env::current_dir().expect("cwd").to_string_lossy(),
        "mcpServers": []
    })).await;
    assert!(loaded.get("result").is_some(), "session was not retained: {loaded}");
