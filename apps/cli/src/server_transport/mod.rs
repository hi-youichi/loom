//! HTTP/SSE transport layer for communicating with loom-server.
//!
//! Provides a typed, reusable client for the loom-server REST API and
//! Server-Sent Events (SSE) channels. Designed for CLI integration where
//! the `loom` binary connects to a running `loom-server` instance.
//!
//! # Core types
//!
//! - [`LoomServerClient`] — high-level client combining HTTP and SSE.
...
//! # Session API
//!
//! The session CRUD and run endpoints map directly to loom-server routes:
//!
...
/// The SSE heartbeat interval published by loom-server.
///
