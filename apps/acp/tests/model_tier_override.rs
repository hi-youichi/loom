...
use common::{create_agent_config, write_last_model_file, TestEnvironment};
...
async fn test_acp_model_overrides_agent_tier() {
    ...
    assert!(
        response.to_string().contains("[TIER:high]"),
        "Mock response should contain tier indicator"
    );
...
async fn test_agent_tier_respected_without_user_selection() {
    ...
    assert!(
        ...
        "Config should contain medium tier"
    );
    ...
    assert_eq!(
        ...
        "Model should map to medium tier"
    );
...
async fn test_model_tier_priority_chain() {
    ...
    assert_eq!(
        high_tier,
        Some("high".to_string()),
        "User selection should result in high tier"
    );
