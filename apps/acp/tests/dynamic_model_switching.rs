...
async fn test_dynamic_model_switching() {
    ...
    assert_eq!(
        ...
        "Initial model should be medium tier"
    );
    ...
    assert_eq!(
        new_tier,
        Some("high".to_string()),
        "Switched model should be high tier"
    );
...
async fn test_multiple_dynamic_switches() {
    ...
    println!(
        "✅ Multiple switches test passed - model handles multiple dynamic switches correctly"
    );
}
...
async fn test_rapid_switching_stability() {
    ...
    for model in models {
        ...
        assert!(
            tier.is_some(),
            "Model {} should have a valid tier mapping",
            model
        );
    }
