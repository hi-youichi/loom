...
const env = {
  ...
  ANUREO_PORT: '3902',
  ANUREO_HMR_UI_PORT: '5180',
  ANUREO_HMR_HOST: '127.0.0.1',
  ANUREO_DISABLE_PWA_DEV: '1',
};

const args = ['x', 'vite', '--force', '--host', '127.0.0.1', '--port', '5180', '--strictPort'];

console.log(`[run-anureo-web] cwd=${webRoot}`);
console.log(`[run-anureo-web] vite args=${JSON.stringify(args)}`);
...
child.on('exit', (code, signal) => {
  console.log(`[run-anureo-web] child exited code=${code} signal=${signal}`);
  process.exit(code ?? 1);
});