    ...
    S: Clone + Send + Sync + Debug + 'static,
{
    ...
    ///
    /// ```rust,ignore
    /// use loom_graph::StateGraph;
    /// use loom_graph::channels::FieldBasedUpdater;
    /// use std::sync::Arc;
    ///
    /// #[derive(Clone, Debug)]
    ...
    ///
    /// ```rust,no_run
    /// use loom_graph::{StateGraph, RetryPolicy};
    ...
    ///
    /// ```rust,no_run
    /// use loom_graph::{StateGraph, DefaultInterruptHandler};
    /// use std::sync::Arc;
    ///
    /// let graph = StateGraph::<String>::new()
