...
async function captureGifFrame(tabId, action) {
  if (tabGroupId === null) return;
  const storageKey = `gif_recording_${tabGroupId}`;
  try {
    const data = await chrome.storage.local.get(storageKey);
    const rec = data[storageKey];
    if (!rec || rec.stoppedAt) return;
    const { base64 } = await takeScreenshot(tabId);
    rec.frames.push({ base64, action, timestamp: Date.now() });
    if (rec.frames.length > 50) rec.frames.shift();
    await chrome.storage.local.set({ [storageKey]: rec });
  } catch {}
}
  ...
  async computer(args) {
    ...
    switch (action) {
      case "screenshot": {
        ...
        await captureGifFrame(tabId, action);
        return {
          content: [
    ...
    if (ref) {
      const evalResult = await cdp(tabId, "Runtime.evaluate", {
        expression: `(() => {
          ...
          if (!el) return { error: "Element not found for ref: ${ref}" };
          if (el.tagName.toLowerCase() !== "input" || el.type !== "file") {
            return { error: "Element is not a file input. Got: " + el.tagName + (el.type ? "[" + el.type + "]" : "") };
          }
      ...
      if (val?.error) return { content: [{ type: "text", text: val.error }] };
      if (!val?.ok) return { content: [{ type: "text", text: "Could not resolve ref to a file input element." }] };

      const { objectId } = await cdp(tabId, "Runtime.evaluate", {
        expression: `document.querySelector('input[type="file"]') === window.__unblockedChrome?.resolveRef?.("${ref}") ? (() => {
          const el = window.__unblockedChrome?.resolveRef?.("${ref}");
          el.dataset.__uploadTarget = "true";
          return el;
        })() : window.__unblockedChrome?.resolveRef?.("${ref}")`,
        returnByValue: false,
      });

      if (!objectId) return { content: [{ type: "text", text: "Could not get DOM node reference for file input." }] };

      const describeResult = await cdp(tabId, "DOM.describeNode", { objectId });
      const backendNodeId = describeResult?.node?.backendNodeId;

      if (!backendNodeId) return { content: [{ type: "text", text: "Could not get backend node ID for file input." }] };
      ...
      return { content: [{ type: "text", text: `Uploaded ${filename} to file input [${ref}]. Change event dispatched.` }] };
    }
    ...
    if (coordinate) {
      ...
      await cdp(tabId, "Runtime.evaluate", {
        expression: `(() => {
          ...
          return "not_file_input:" + (targetEl ? targetEl.tagName : "null");
        })()`,
        returnByValue: true,
      });

      await sleep(500);
      return { content: [{ type: "text", text: `Attempted upload at (${x}, ${y}) as ${filename}.` }] };
    }

    return { content: [{ type: "text", text: "Provide either ref (file input element) or coordinate to upload the image." }] };
  },

  async gif_creator(args) {
    const { action, tabId, filename, download, options } = args;
    if (!(await isInGroup(tabId))) return { content: [{ type: "text", text: `Tab ${tabId} is not in the MCP group.` }] };
    ...
    switch (action) {
      case "start_recording": {
        await chrome.storage.local.set({ [storageKey]: { frames: [], startTime: Date.now(), actions: [] } });
        return { content: [{ type: "text", text: "GIF recording started. Use computer tool actions normally - key actions will be tracked. Stop with action: stop_recording." }] };
      }

      case "stop_recording": {
        const data = await chrome.storage.local.get(storageKey);
        const rec = data[storageKey];
        if (!rec) return { content: [{ type: "text", text: "No active recording for this tab group." }] };
        rec.stoppedAt = Date.now();
        await chrome.storage.local.set({ [storageKey]: rec });
        return { content: [{ type: "text", text: `Recording stopped. ${rec.frames.length} frame(s) captured, ${rec.actions.length} action(s) tracked. Use action: export to generate GIF.` }] };
      }
      ...
      case "export": {
        ...
        if (!rec || rec.frames.length === 0) {
          return { content: [{ type: "text", text: "No recorded frames to export. Start and stop a recording first." }] };
        }

        const ts = filename || `recording-${Date.now()}.gif`;
        const frameCount = rec.frames.length;
        const actionCount = rec.actions.length;

        const summary = `GIF export: ${frameCount} frames, ${actionCount} actions tracked.\nFrames are stored as JPEG screenshots in chrome.storage.local.\nFull GIF encoding requires an external tool — the frames and action log are available for processing.\nFilename: ${ts}`;

        if (download) {
          const resp = await sendContentMessage(tabId, {
            type: "downloadGifFrames",
            frames: rec.frames,
            actions: rec.actions,
            filename: ts,
            options: options || {},
          });
          return { content: [{ type: "text", text: resp?.result || summary }] };
        }
      ...
      case "clear": {
        ...
        return { content: [{ type: "text", text: "Recording cleared." }] };
      }
