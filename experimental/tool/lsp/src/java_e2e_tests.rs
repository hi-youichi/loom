...
mod java_e2e_tests {
    ...
    use config::{get_default_lsp_servers, load_default_lsp_config, LspConfig};
    use std::path::Path;
    ...
    fn test_java_e2e_config_serialization() {
        ...
        // 2. 验证Java配置存在
        assert!(
            config.servers.iter().any(|s| s.language == "java"),
            "Default config should contain Java server"
        );

        // 3. 验证配置可以被正确处理（通过load_default_lsp_config）
        let loaded_config =
            load_default_lsp_config().expect("Default config should load successfully");

        // 4. 验证加载的配置包含Java
        assert!(
            loaded_config.servers.iter().any(|s| s.language == "java"),
            "Loaded config should contain Java server"
        );

        // 5. 验证两个配置的Java部分一致
        let original_java = config
            .servers
            .iter()
            .find(|s| s.language == "java")
            .unwrap();
        let loaded_java = loaded_config
            .servers
            .iter()
            .find(|s| s.language == "java")
            .unwrap();
        ...
        assert_eq!(
            original_java.startup_timeout_ms,
            loaded_java.startup_timeout_ms
        );
    }
    ...
    fn test_java_e2e_installer_integration() {
        ...
        assert!(
            ...
            "Java should be supported by installer"
        );
    ...
    fn test_java_e2e_config_loading_flow() {
        ...
        // 1. 加载默认配置
        let loaded_config =
            load_default_lsp_config().expect("Default LSP config should load successfully");

        // 2. 验证配置包含Java
        assert!(
            !loaded_config.servers.is_empty(),
    ...
    fn test_java_e2e_multi_file_scenario() {
            ...
            match file_path {
                path if path.ends_with(".java") => {
                    assert_eq!(
                        detected,
                        Some("java".to_string()),
                        "Java file '{}' should be detected as Java",
                        path
                    );
                    java_count += 1;
                }
                _ => {
                    assert_ne!(
                        detected,
                        Some("java".to_string()),
                        "Non-Java file '{}' should not be detected as Java",
                        file_path
                    );
                    non_java_count += 1;
                }
        ...
        assert!(
            java_count > 0,
            "Should have detected at least one Java file"
        );
        assert!(
            non_java_count > 0,
            "Should have correctly rejected non-Java files"
        );
    }
    ...
    fn test_java_e2e_performance_characteristics() {
        ...
        assert!(
            config_load_time.as_millis() < 100,
            "Config loading should be fast (< 100ms), took: {:?}",
            config_load_time
        );
        ...
        assert!(
            manager_creation_time.as_millis() < 50,
            "Manager creation should be fast (< 50ms), took: {:?}",
    ...
    fn test_java_e2e_configuration_completeness() {
        ...
        // 验证所有必需字段都存在且有效
        assert!(
            !java_config.language.is_empty(),
            "Language should not be empty"
        );
        assert!(
            !java_config.command.is_empty(),
            "Command should not be empty"
        );
        assert!(
            !java_config.file_patterns.is_empty(),
            "File patterns should not be empty"
        );
        assert!(
            java_config.startup_timeout_ms > 0,
        ...
        // 验证可选配置的存在
        assert!(
            java_config.auto_install.is_some(),
            "Java should have auto-install config"
        );
        assert!(
            java_config.initialization_options.is_some(),
            "Java should have initialization options"
        );
        ...
        assert!(
            !auto_install.command.is_empty(),
            "Auto-install command should not be empty"
        );
        assert!(
            auto_install.verify_command.is_some(),
            "Auto-install should have verify command"
        );
    }
}
