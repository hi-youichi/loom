...
mod tests {
    ...
    fn test_lsp_manager_java_config_loaded() {
        ...
        assert!(
            java_config.is_some(),
            "Java config should exist in default configs"
        );
    ...
    fn test_lsp_manager_java_extension_mapping() {
        ...
        for file in test_files {
            ...
            assert_eq!(
                detected,
                Some("java".to_string()),
                "File '{}' should be detected as Java",
                file
            );
        }
    }
    ...
    fn test_lsp_manager_all_supported_languages() {
        let configs = get_configs();
        let supported_languages: Vec<&str> = configs.iter().map(|c| c.language.as_str()).collect();
    ...
    fn test_lsp_manager_java_file_patterns() {
        ...
        assert!(
            ...
            "Java should have file patterns"
        );
        assert!(java_config.file_patterns.contains(&"*.java".to_string()));
    ...
    fn test_case_sensitive_detection() {
        ...
        for (file_path, expected_lang) in test_cases {
            ...
            assert!(
                detected.is_some() || detected == Some(expected_lang.to_string()),
    ...
    fn test_java_vs_other_extensions() {
        ...
        for file_path in unsupported_files {
            ...
            if let Some(lang) = detected {
                assert_ne!(
                    lang, "java",
                    "File '{}' should not be detected as Java, got: {}",
                    file_path, lang
                );
            }
        }
    ...
    fn test_path_variations() {
        ...
        for path in path_variations {
            ...
            assert_eq!(
                detected,
                Some("java".to_string()),
                "Java file should be detected in path: {}",
                path
            );
        }
    }
