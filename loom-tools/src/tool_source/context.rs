...
/// - `thread_id`: Optional thread/session id from [`RunnableConfig`](loom_memory::RunnableConfig); set by ActNode when running with RunContext. Use for session-scoped storage (e.g. todo per thread).
