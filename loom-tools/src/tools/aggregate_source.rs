...
///
/// ```no_run
/// use loom_tools::tools::{AggregateToolSource, Tool};
/// use loom_tools::tool_source::{ToolCallContent, ToolCallContext, ToolSourceError, ToolSpec, ToolSource};
/// # use async_trait::async_trait;
/// # struct MockTool;
...
impl AggregateToolSource {
    ...
    /// # #[tokio::main]
    /// # async fn main() {
    /// use loom_tools::tools::AggregateToolSource;
...
impl ToolSource for std::sync::Arc<AggregateToolSource> {
    ...
    async fn call_tool(
        ...
        self.as_ref().call_tool(name, arguments, None).await
    }
}
