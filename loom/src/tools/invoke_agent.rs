...
async fn invoke_single_agent(
    ...
    let worktree_handle = if use_worktree && working_folder_override.is_none() {
        ...
        match crate::worktree::WorktreeManager::from_working_dir(current_dir, worktree_config) {
            Ok(manager) => {
                match manager
                    .create_for_agent(
                        agent_name,
...
tracing::debug!(agent = %agent_name, "Starting sub-agent execution");
    ...
    if let Some(handle) = worktree_handle {
        let wt_config = profile
            ...
            .map(|wc| wc.to_worktree_config())
            .unwrap_or_default();
        let manager = crate::worktree::WorktreeManager::new(handle.repo_root.clone(), wt_config.clone());
