...
/// Spawn a background review in a new thread (Hermes 对齐).
///
/// 使用 std::thread::spawn 而非 tokio::spawn，确保：
/// - 主进程退出时 review 线程继续执行
/// - 静默运行，不污染用户终端
///
/// Hermes 等价实现：
/// ```python
...
pub fn spawn_background_review(
    ...
    std::thread::spawn(move || {
        // 创建独立的 tokio runtime（不依赖主进程 runtime）
        let rt = tokio::runtime::Runtime::new().unwrap();
        
        let start = std::time::Instant::now();
        let result = rt.block_on(async {
            run_background_review_workflow(&config, &session_content, &session_id).await
        });
