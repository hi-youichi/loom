...
use loom::model_spec::{ModelsDevResolver, ModelLimitResolver};
...
async fn main() {
    ...
    if let Some(spec) = resolver.resolve("anthropic", "claude-3-5-sonnet-20241022").await {
    ...
    if let Some(model) = resolver.fetch_model("google", "gemini-2.0-flash").await {
        ...
        println!("   Context: {} tokens", model.limit.as_ref().map(|l| l.context).unwrap_or(0));
    ...
    if let Ok(providers) = resolver.fetch_all_providers().await {
        ...
        println!("   ... and {} more vision models", vision_count.saturating_sub(3));
        println!("   Total: {} vision, {} audio, {} video models", 
