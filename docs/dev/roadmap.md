...
- **2026-06-23**：批量完成 P3/P4 任务——新增 #35 `threat_patterns`、#36 `context_fence`（memory-v2 crate）；验证标记 #38（错误类型重构已实现）、#40（菱形依赖已解决）、#42（首次运行延迟已实现）、#28（absorbed_into prompt 已实现）；实现 #39（streaming clone 优化）；执行 #37 死代码清理 batch 1+2（零 clippy 警告）。
