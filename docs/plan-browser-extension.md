...
1. **Windows 安装脚本** — 编写 `install.ps1` 自动注册 native messaging host
2. **Agent Profile** — 创建 `.loom/agents/browser/` 封装常用操作组合
3. **Skill** — 创建 `.loom/skills/browser-automation.md` 提供 Agent 使用指导
4. **WebSocket 方案** — 去掉 native messaging，简化安装（见附录 A）
